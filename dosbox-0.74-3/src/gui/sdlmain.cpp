...
/*
Modified by Lakhya Jyoti Nath (ljnath) for GUI Turbo Assmebler X64
Contact: https://ljnath.com | ljnath@ljnath.com
*/
...
void GFX_SetTitle(Bit32s cycles,Bits frameskip,bool paused){
	...
	/*guiTasmX64: below comments the existing DOSBox dynamic title and sets GUI Turbo Assembler X64 as the title*/
	...
	SDL_WM_SetCaption("GUI Turbo Assembler x64",VERSION);
}
...
#endif
	/*guiTasmX64: don't set any logo here; regardless of wherever this function is called*/
...
static void GUI_StartUp(Section * sec) {
	/*guiTasmX64 - start DOSBox minimized using SDL_WM_IconifyWindow when -minimized is specified*/
	if(control->cmdline->FindExist("-minimized")){
   ...
   if(sdl.desktop.want_type==SCREEN_OPENGL){ /* OPENGL is requested */
	...
	GFX_Stop();
	// guiTasmX64 - changing the program title
	SDL_WM_SetCaption("GUI Turbo Assembler x64",VERSION);
...
//#endif
	...
	/*guiTasmX64 - remove the splash screen*/
...
int main(int argc, char* argv[]) {
	try {
		...
		/*guiTasmX64: hiding the console screen; as such the -noconsole flag becomes obsolete*/
		FreeConsole();
